...
| **API Key** | 在 Metapi UI 创建的[下游 API Key](./configuration.md#下游-api-key-策略) |
...
| 配置项 | 值 |
|--------|-----|
| Settings → Models → OpenAI API Key | 下游 API Key |
...
```json
{
  "env": {
    ...
    "ANTHROPIC_API_KEY": "sk-your-downstream-key",
    "ANTHROPIC_AUTH_TOKEN": "sk-your-downstream-key",
...
| `ANTHROPIC_API_KEY` | 认证令牌，填下游 API Key |
...
```json
{
  "OPENAI_API_KEY": "sk-your-downstream-key"
}
```
...
你可以在「下游密钥」页面创建多个项目级下游 Key，每个 Key 可独立配置过期时间、费用/请求上限、模型白名单、路由白名单和站点倍率，适用于多团队/多项目共用一个 Metapi 实例的场景。
