    ...
    while (retryCount <= MAX_RETRIES) {
      ...
      try {
        ...
        if (!upstream.ok) {
          ...
          logProxy(selected, requestedModel, 'failed', upstream.status, Date.now() - startTime, errText, retryCount, downstreamApiKeyId, 0, 0, 0, 0, null, clientContext, downstreamPath);
