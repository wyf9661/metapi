...
export type SiteColumnCompatibilitySpec = {
  /** Table the column belongs to; defaults to `sites`. */
  table?: string;
  column: string;
...
export const SITE_COLUMN_COMPATIBILITY_SPECS: SiteColumnCompatibilitySpec[] = [
  {
    // Per-key disabled models. NULL keeps the site-wide semantics of older rows.
    table: 'site_disabled_models',
    column: 'account_id',
    addSql: {
      sqlite: 'ALTER TABLE site_disabled_models ADD COLUMN account_id integer REFERENCES accounts(id) ON DELETE cascade;',
      mysql: 'ALTER TABLE `site_disabled_models` ADD COLUMN `account_id` INT NULL',
      postgres: 'ALTER TABLE "site_disabled_models" ADD COLUMN "account_id" INTEGER REFERENCES "accounts"("id") ON DELETE CASCADE',
    },
  },
  {
    column: 'proxy_url',
...
export const SITE_TABLE_COMPATIBILITY_SPECS: SiteTableCompatibilitySpec[] = [
  ...
  },
  {
    table: 'site_disabled_models',
    createSql: {
      sqlite: 'CREATE TABLE IF NOT EXISTS site_disabled_models (id integer PRIMARY KEY AUTOINCREMENT NOT NULL, site_id integer NOT NULL REFERENCES sites(id) ON DELETE cascade, account_id integer REFERENCES accounts(id) ON DELETE cascade, model_name text NOT NULL, created_at text DEFAULT (datetime(\'now\')));',
      mysql: 'CREATE TABLE IF NOT EXISTS `site_disabled_models` (`id` INT AUTO_INCREMENT PRIMARY KEY, `site_id` INT NOT NULL, `account_id` INT NULL, `model_name` VARCHAR(191) NOT NULL, `created_at` TEXT NULL, CONSTRAINT `site_disabled_models_site_fk` FOREIGN KEY (`site_id`) REFERENCES `sites`(`id`) ON DELETE CASCADE)',
      postgres: 'CREATE TABLE IF NOT EXISTS "site_disabled_models" ("id" INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "site_id" INTEGER NOT NULL REFERENCES "sites"("id") ON DELETE CASCADE, "account_id" INTEGER REFERENCES "accounts"("id") ON DELETE CASCADE, "model_name" TEXT NOT NULL, "created_at" TEXT)',
    },
    postCreateSql: {
      // The pre-per-key unique index (site_id, model_name) would forbid both a
      // site-wide row and a per-key row for the same model, so it is replaced.
      sqlite: [
...
function isExistingSchemaObjectError(error: unknown): boolean {
  ...
  return lowered.includes('duplicate column')
    ...
    // MySQL has no DROP INDEX IF EXISTS: re-running the drop reports the index
    // as missing, which is the state we asked for.
    || lowered.includes('check that column/key exists')
